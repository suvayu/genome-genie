...
For each task in the pipeline specification, one or more jobs maybe
created (equal to the number of input files).  These jobs are
essentially `SGE` job scripts, that are submitted to the cluster using
`qsub`.

Different parts of the final job script are generated by the different
templates that are available to the pipeline script, and they are put
together by the `jobscript` template.  The `job_header` comes from
`sge`, `setup` comes from `module`, and `job_cmd` comes from the tool
specific template, e.g `gatk`.
